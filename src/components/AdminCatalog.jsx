...
export default function AdminCatalog() {
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="font-body text-sm font-semibold text-dark mb-1.5 block">Nombre</label>
          ...
          <div>
            <label className="font-body text-sm font-semibold text-dark mb-1.5 block">Precio normal (CLP)</label>
            <input type="number" className={inp} placeholder="20000"
              value={form.price} onChange={(e) => setField("price", e.target.value)} />
          </div>
          <div>
            <label className="font-body text-sm font-semibold text-dark mb-1.5 block">Categoría</label>
          ...
          {/* Oferta */}
          <div className="sm:col-span-2 bg-neutral-50 rounded-xl p-4 flex flex-col gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.on_sale}
                onChange={(e) => setField("on_sale", e.target.checked)}
                className="w-4 h-4 accent-primary" />
              <span className="font-body text-sm font-semibold text-dark">Poner en oferta</span>
            </label>
            {form.on_sale && (
              <div>
          ...
          <div className="sm:col-span-2">
            <label className="font-body text-sm font-semibold text-dark mb-1.5 block">
              Tallas <span className="text-neutral-500 font-normal">(separadas por coma)</span>
            </label>
          ...
          <div className="sm:col-span-2">
            <label className="font-body text-sm font-semibold text-dark mb-1.5 block">URL de imagen</label>
            <input className={inp} placeholder="https://..."
              value={form.image} onChange={(e) => setField("image", e.target.value)} />
            {form.image && (
              <div className="mt-2 w-20 h-20 rounded-xl overflow-hidden border border-neutral-200">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={form.active}
            ...
            className="w-4 h-4 accent-primary" />
          <span className="font-body text-sm text-dark">Visible en la tienda (activo)</span>
        </label>

        <button onClick={handleSave} disabled={saving}
          className="w-full bg-primary hover:bg-primary/90 disabled:opacity-60 text-white font-bold rounded-full py-3.5 transition-all text-sm">
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h3 className="font-display font-bold text-lg text-dark">Catálogo</h3>
        <button onClick={openNew}
          className="bg-primary hover:bg-primary/90 text-white font-bold rounded-full px-5 py-2.5 text-sm transition-all">
          Nuevo producto
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-neutral-200 p-12 text-center">
          <p className="font-display font-bold text-lg text-dark mb-2">Catálogo vacío</p>
          <p className="font-body text-sm text-neutral-500">Agrega tu primer producto.</p>
        </div>
      ) : (
          ...
          {products.map((p) => (
            <div key={p.id}
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-12 h-12 rounded-xl overflow-hidden bg-neutral-50 flex-shrink-0 border border-neutral-200/30">
                  {p.image && <img src={p.image} alt={p.name} className="w-full h-full object-cover" />}
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-display font-bold text-sm text-dark truncate">{p.name}</span>
                    {p.on_sale && (
                      <span className="text-xs font-bold rounded-full px-2 py-0.5 bg-primary text-white">
                        ¡Oferta!
                      </span>
                    )}
                    {!p.active && (
                      <span className="text-xs font-semibold rounded-full px-2 py-0.5 bg-neutral-50 text-neutral-500 border border-neutral-200">
                        Oculto
                      </span>
                    )}
                  </div>
                  <p className="font-body text-xs text-neutral-500 mt-0.5">
                    {p.on_sale && p.sale_price
                      ? <>
                          ...
                          {" "}<span className="text-red-500 font-semibold">{fmtCLP(p.sale_price)}</span>
                        </>
                      : fmtCLP(p.price)}
