...
const PRODUCTS = [
  ...
  {
    id: 2,
    name: "Scrunchie de Terciopelo Rosa Palo",
    price: 3490,
  ...
  {
    id: 3,
    name: "Set de Pinches Perla × 6 — Coquette Edition",
    price: 6990,
...
const CATEGORIES = [
  { label: "Lazos",       emoji: "🎀", href: "#" },
  { label: "Scrunchies",  emoji: "💫", href: "#" },
  { label: "Pinches",     emoji: "✨", href: "#" },
  { label: "Sets",        emoji: "🌸", href: "#" },
  { label: "Novedades",   emoji: "💝", href: "#" },
];
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-5 animate-fade-up">
            ...
            <p className="font-body text-coquette-mist text-base sm:text-lg leading-relaxed max-w-md">
              Lazos, scrunchies y pinches artesanales para mujeres que abrazan su feminidad con orgullo.
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto mt-1">
              <a
                href="#catalogo"
                className="inline-flex items-center justify-center gap-2 rounded-full py-3 px-8 bg-coquette-burgundy text-white font-body font-semibold text-sm shadow-bloom hover:bg-coquette-mauve hover:-translate-y-0.5 transition-all duration-250 active:translate-y-0"
              >
                Ver Catálogo
                <ArrowRight size={15} />
              </a>
              <a
                href="#novedades"
                className="inline-flex items-center justify-center gap-2 rounded-full py-3 px-8 bg-transparent border border-coquette-rose text-coquette-burgundy font-body font-semibold text-sm hover:bg-coquette-blush transition-all duration-250"
              >
                Novedades ✨
              </a>
            </div>
            ...
            <div className="flex gap-8 mt-3">
              {[
                { value: "+2.4k", label: "Clientas felices" },
                ...
                <div key={s.label} className="flex flex-col items-center lg:items-start gap-0.5">
                  ...
                  <span className="font-body text-coquette-mist text-2xs">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {CATEGORIES.map((cat) => (
            <a
              key={cat.label}
              href={cat.href}
              className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 bg-white border border-coquette-dust/50 font-body text-sm text-coquette-mist hover:bg-coquette-blush hover:border-coquette-rose hover:text-coquette-burgundy transition-all duration-250 shadow-soft hover:shadow-petal"
            >
              <span>{cat.emoji}</span>
              {cat.label}
            </a>
    ...
    <section id="catalogo" className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <p className="font-body text-xs text-coquette-mauve tracking-widest uppercase mb-2">
              ✨ Curados para ti
            </p>
            <h2 className="font-display text-coquette-ink text-3xl sm:text-4xl leading-tight">
              Productos Destacados
            </h2>
          </div>
          <a
            href="#"
            className="inline-flex items-center gap-1.5 font-body text-sm text-coquette-burgundy hover:text-coquette-mauve transition-colors duration-250 group"
          >
...
function PromoBanner() {
  return (
    <section className="mx-4 sm:mx-6 lg:mx-8 my-4 rounded-4xl overflow-hidden bg-coquette-gradient relative">
      ...
      <div className="relative max-w-6xl mx-auto px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
        <div>
          <p className="font-display text-coquette-ink text-2xl sm:text-3xl leading-snug max-w-md">
            ...
            <span className="text-coquette-burgundy">$15.000</span> 🎀
          </p>
          <p className="font-body text-coquette-mist text-sm mt-1">
            Solo por esta semana · Envíos a todo Chile
          </p>
        </div>
        <a
          href="#catalogo"
          className="shrink-0 inline-flex items-center gap-2 rounded-full py-3 px-7 bg-coquette-burgundy text-white font-body font-semibold text-sm shadow-bloom hover:bg-coquette-mauve hover:-translate-y-0.5 transition-all duration-250"
        >
          Aprovechar oferta
          <ArrowRight size={15} />
        </a>
